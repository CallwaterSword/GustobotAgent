...
from .validators import (
    ...
    validate_labels_and_types_in_cypher_query,
...
def create_text2cypher_validation_node(
    ...
    async def validate_cypher(state: CypherState) -> Dict[str, Any]:
        ...
        # P0 安全修复：label/relationship type 白名单校验
        # 阻止 LLM 生成的 Cypher 引用 schema 之外的未知/恶意标签或关系类型
        whitelist_errors = validate_labels_and_types_in_cypher_query(
            graph=graph, cypher_statement=state.get("statement", "")
        )
